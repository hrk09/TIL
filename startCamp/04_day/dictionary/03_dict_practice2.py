...
"""
# count = 0
# for location in ssafy.keys():
#     count += len(location)

# print(count)

count = len(ssafy['location'])
print(count) # print(len(ssafy['location']))도 가능하다.
...
for library in ssafy['language']['python']['python standard library']:
    #['os', 'random', 'webbrowser']
    if library == 'requests':
        flag = True

print(flag)

# print('requests' in ssafy['language']['python']['python standard library']) 도 가능하다.
...
import random
d1 = random.choice(ssafy['classes']['dj']['groups']['E'])
print(f'오늘의 당번은 {d1}')
